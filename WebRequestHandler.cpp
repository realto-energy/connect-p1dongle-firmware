...
void WebRequestHandler::handleRequest(AsyncWebServerRequest *request)
{
  ...
  update_autoCheck, update_auto, pls_en, pls_emu, dev_fleet, alpha_fleet, v2_fleet, spiffsMounted;
  if(request->url() == "/"){
  ...
  if(mqtt_en){
    ...
    if(request->hasParam("v2_fleet")){
      AsyncWebParameter* p = request->getParam("v2_fleet");
